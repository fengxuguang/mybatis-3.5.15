...
public class MapperAnnotationBuilder {
    ...
    public void parse() {
        ...
        if (!configuration.isResourceLoaded(resource)) {
            // 检测是否加载过对应的映射配置文件, 如果未加载, 则创建 XMLMapperBuilder 对象解析对应的映射文件
            loadXmlResource();
            // loadedResources 集合添加数据
            ...
            for (Method method : type.getMethods()) {
                ...
                try {
                    // 解析 @SelectKey 注解, 并将 mappedStatement 添加到 configuration 中的 mappedStatements 属性中
                    parseStatement(method);
                } catch (IncompleteElementException e) {
                    // 如果解析过程出现 IncompleteElementException 则将这个方法放入 incompleteMethods 集合中
