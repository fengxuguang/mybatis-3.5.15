...
public class XMLMapperBuilder extends BaseBuilder {
	...
	/**
	 * 解析 SQL 的 Mapper 映射文件
	 */
	public void parse() {
		// 判断是否已经加载过该映射文件
		if (!configuration.isResourceLoaded(resource)) {
			// 语句注册, 具体的增删改查接口标签解析 <insert> <update> <delete> <select>, 一个标签一个 MappedStatement 对象
			configurationElement(parser.evalNode("/mapper"));
			// 接口注册, 把 namespace (接口类型) 和工程类绑定起来, 放到一个 map. 一个 namespace, 一个 MapperProxyFactory
			// 将 resource 添加到 Configuration.loadedResources 集合中保存, loadedResources 集合类型是 HashSet, 记录了已经加载过的映射文件
			configuration.addLoadedResource(resource);
			// 绑定映射器 namespace
			bindMapperForNamespace();
		}

		// 处理 ConfigurationElements 方法中解析失败的 resultMap 节点
		parsePendingResultMaps();

		// 处理 ConfigurationElements 方法中解析失败的 cache-ref 节点
		parsePendingCacheRefs();

		// 处理 ConfigurationElements 方法中解析失败的 SQL 语句节点
	...
	private void configurationElement(XNode context) {
		try {
			...
			// namespace 没设置 或 值为空字符时抛异常
			if (namespace == null || namespace.isEmpty()) {
				throw new BuilderException("Mapper's namespace cannot be empty");
			}
			// 将 namespace 设置到 MapperBuilderAssistant 的 currentNamespace 字段, 记录当前命名空间
	...
	private ResultMap resultMapElement(XNode resultMapNode, List<ResultMapping> additionalResultMappings,
		...
		// 获取 resultMap 节点的子节点列表, 如: <id> <result> <constructor> 等标签
